...
const mainTemp = handlebars.compile(mainSrc);
 ...
 function loadData(template,options, res) {
...
get: function(uri, queryParams, querySearch,urlAll, req, res) {
    ...
    if(uri == '/alert/delete/true/') {
        ...
        this.deleteData(mainTemp, req, res,options) ;
...
post: function(uri, queryParams, req, res) {
    ...
    if(uri == "/alert"){
        ...
        this.insertData(mainTemp,req,res,options);
    }
    
    if(uri == "/alert/edit/true/") {
        handlebars.registerPartial('body', fs.readFileSync(path.join(process.cwd(),'templates/read.hbs'), 'utf-8'));
        const options ={
            add: "/alert/add",
            togglePage:"/machine",
        ...
        this.updateRecord(mainTemp, req, res,options) ;
    }
...
insertData: function (template,req, res,options) {
...
updateRecord: function(template, req, res,options) {
    ...
    this.collectRequestData(req, result => {
        ...
        console.log(result._id) ;
            ...
            { new: true },
            (err, doc) => {
                if (!err) { loadData(template,options, res); }
                else {
...
deleteData: function(template, req, res,options) {
